...
# M-01（Java。JaCoCo）/ M-11・M-12（テストの結果）: バックエンドのビルドとテスト。
# measure.sh が source する（単独では実行しない）。

measure_backend() {
  local dir="$SRC/$BACKEND_DIR" mvn exec
  [ -f "$dir/pom.xml" ] || { fail "M-01/M-11: $BACKEND_DIR/pom.xml がありません"; return; }
  ...
  # テストの失敗では止めない。失敗したテストも M-11 の判定材料として送る
  if ! (cd "$dir" && "${mvn[@]}" -B -ntp \
    ...
    fail "バックエンドのビルドに失敗しました（M-01 Java / M-11 は送られません）"
  fi
  endgroup
