...
- 性能指標（M-03〜05）は専有環境で計測したときだけ判定に使う（D-7）。GitHub ホストランナーでの計測値は
  `REFERENCE`（参考値）になる（[docs/02-metrics-spec.md](docs/02-metrics-spec.md)「ランナー種別の影響」）
...
（[docs/02-metrics-spec.md](docs/02-metrics-spec.md)「計測条件」）、
...
| Secrets | `QG_INGEST_TOKEN` | `qg_<prefix>_<secret>` | リポジトリ単位の Ingest Token（前節を参照） |
...
| `QG_SCHEDULE_ZONE` | `Asia/Tokyo` | 日次バッチ（02:00 再評価・02:10 免除の期限切れ・03:00 保持期間・03:10 滞留 Run・09:00 鮮度確認）のタイムゾーン |
