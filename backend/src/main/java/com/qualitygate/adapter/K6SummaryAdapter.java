...
/**
 * k6 の summary JSON から M-03 / M-04 / M-05 を読む（docs/02-metrics-spec.md M-03）。
 *
...
public class K6SummaryAdapter implements ArtifactAdapter {
    ...
    public NormalizedReport parse(InputStream in, ParseContext context) {
        ...
        if (context.metadataFlag("aborted")) {
            // 部分的な結果で判定しない（docs/02-metrics-spec.md M-03 境界条件）
     ...
     *
     * <p>ウォームアップ（60 秒）は集計から除く（docs/02-metrics-spec.md M-03 計測条件）。
