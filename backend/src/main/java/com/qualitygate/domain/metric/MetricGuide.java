...
public record MetricGuide(
    ...
    private static final List<MetricGuide> ALL = List.of(
            ...
            new MetricGuide("M-08",
                    "画面とサーバの間の「約束事（API の仕様）」が守られているか",
                    Basis.TEAM_DECISION,
                    "約束事は 1 つでも破れると連携が壊れるため、すべて成功（100%）を求める。",
                    "画面が表示できない、データが保存されないなどの不具合につながる。",
                    "API 契約テストの成功率 = 成功 / 実行 × 100。実行件数が最小件数に満たなければ不合格。"),
