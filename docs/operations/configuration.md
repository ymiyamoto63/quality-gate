...
| `QG_RETENTION_RUN_DAYS` / `QG_RETENTION_ARTIFACT_DAYS` / `QG_RETENTION_AUDIT_LOG_DAYS` | `730` / `90` / `730` | データ保持期間（日）。Run・指標値・違反 / 成果物のファイル / 監査ログ。過ぎたデータは毎日の保持期間バッチで削除される。下限（30 / 1 / 365 日）を下回るとアプリが起動しない |
