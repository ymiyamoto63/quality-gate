...
| カテゴリ | 主な指標 | 合格ライン |
| --- | --- | --- |
| 機能テスト | ブランチカバレッジ / ミューテーションスコア / テスト成功率 / スキップされたテスト数 | 75% / 60% 以上（ミューテーションは backend のみ）/ 100% / 前回から増やさない（テストの 2 指標は設定で有効にしたときだけ） |
...
| 契約・互換性 | API 契約テスト成功率 / OpenAPI の破壊的変更件数 | 100% / 0 件 |
...
詳細と選定理由は [技術スタック](initial/04-tech-stack.md) を参照してください。
