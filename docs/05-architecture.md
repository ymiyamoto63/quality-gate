...
```
1. 設定解決     commitSha の .quality-gate.yml を取得・検証し、GateConfig 版を確定
...
| 種別 | 契機 | 内容 |
| --- | --- | --- |
| `EVALUATE_RUN` | `finalize` | 正規化 → 判定（3.2） |
...
finalize
   │
   ├─ GitHub Contents API で commitSha 時点の .quality-gate.yml を取得
   │    │
   │    ├─ 取得できた → スキーマ検証
   │    │      ├─ OK   → 内容の SHA-256 を計算
   │    │      │          ├─ 同一ハッシュの GateConfig が既存 → その版を再利用
   │    │      │          └─ 無し → 新しい版として保存
   │    │      └─ NG   → Run を EVALUATED（全指標 ERROR）とし、検証エラーを理由に記録
   │    │
   │    └─ 取得できない（ファイル無し）→ UI 設定、無ければシステム既定値を使う
   │
   ▼
...
```
.quality-gate.yml（リポジトリ内） > UI 設定 > システム既定値
```

ファイルを優先するのは、設定がコードと同じライフサイクルで管理され、
変更がレビューの対象になるため（Configuration as Code）。
UI 設定は、ファイルを置く前の暫定運用と、ファイルに書かない項目
（通知先など環境依存の値）のために残す。

### 7.3 検証

JSON Schema で `.quality-gate.yml` を検証する。検証エラーは行番号付きで返し、
UI と CI ログの双方で原因が分かるようにする。
**未知のキーはエラーとする**（typo を黙って無視しない）。
