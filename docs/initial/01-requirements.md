...
```
 ...
 quality-gate リポジトリの GitHub Actions（セルフホストランナー）
   plan（計測する先頭コミットを探す）→ fetch（取得）→ measure（計測。認証情報なし）→ submit（Ingest API へ送信）
                                                                  │ HTTPS / Ingest Token
                                                                  ▼
                                        quality-gate（取り込み → 正規化 → 評価 → 可視化 → 通知）
```

| 項目 | 内容 |
| --- | --- |
| きっかけ | 定期実行（15 分ごと。既定ブランチと、同じリポジトリのブランチから出ている PR の先頭のうち未計測のもの）と手動実行 |
