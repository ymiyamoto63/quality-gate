...
**全 10 指標の判定・監査ログ・再評価・全画面まで実装済み（2026-09-23）。免除・通知・定期実行は 2026-09-26 に廃止（D-22）。リリース判定（S-11。D-23）を追加。**
...
| 収集ランナー（対象リポジトリに何も置かない計測。D-16） | 段階 1〜4 を実装（M-01 / M-02 / M-06〜M-17。M-07 は backend と frontend。手動実行のみ（定期実行は D-22 で廃止）。PR も指定すれば計測できる。M-02 は既定ブランチのみ。M-10 は対象アプリを起動して検査。対象のコードは計測用のコンテナの中で動かす。[収集ランナーで計測する](operations/collector.md)） |
...
- リリース判定（S-11。UC-10 / D-23）— タグかコミットを指定し、そのコミットの最新の完全計測から結論（リリース可 / 注意あり / 不可 / 判定できない）と全指標の合否を出す。指標ごとに説明と基準の根拠（外部基準 / 業界の目安 / チーム判断）を示し、CSV を証跡として出力する（出力は監査ログに残る）。タグは GitHub API でコミットに解決する
