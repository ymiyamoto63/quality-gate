...
収集ランナーが計測するのは現状 M-01 / M-02 / M-06〜M-09 で（M-02 は既定ブランチのみ）、M-10 は段階 4 で追加する。
...
| 収集ランナー（対象リポジトリに何も置かない計測。D-16） | 段階 1〜3 を実装（M-01 / M-02 / M-06〜M-09。手動実行と 15 分ごとの定期実行、PR の先頭も計測。M-02 は既定ブランチのみ。[収集ランナーで計測する](operations/collector.md)）。段階 4（M-10）は未着手 |
...
- **収集ランナーの残り** — M-10（アクセシビリティ。段階 4）、計測のコンテナ隔離
