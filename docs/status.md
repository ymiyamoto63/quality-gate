...
**全 10 指標の判定・監査ログ・再評価・全画面まで実装済み（2026-09-23）。免除・通知・定期実行は 2026-09-26 に廃止（D-22）。リリース判定（S-11。D-23）を追加し、タグの計測と前のタグとの比較に対応（D-24）。**
...
| フェーズ | 状態 |
| --- | --- |
| 要件定義 | 完了（v1.9。リリース判定のための計測（D-24）まで反映） |
...
| 収集ランナー（対象リポジトリに何も置かない計測。D-16） | 段階 1〜4 を実装（M-01 / M-02 / M-06〜M-17。M-07 は backend と frontend。手動実行のみ（定期実行は D-22 で廃止）。PR・タグも指定すれば計測できる（タグの比較元は前のタグ。D-24）。M-02 と M-03〜05 は PR 以外。M-10 は対象アプリを起動して検査。対象のコードは計測用のコンテナの中で動かす。[収集ランナーで計測する](operations/collector.md)） |
