...
| `query` は書き込み系モジュール（`ingest` / `normalize` / `evaluate`）を呼ばない | 参照系は保存済みの判定結果を読むだけで、書き込み側の都合に引きずられない |
...
2. 移動の解決   git-renames から比較元コミットからのファイルの移動を読む
...
9. 保存         measurements / findings を置き換え、runs を更新
```
...
| 要件 | 方式 |
| --- | --- |
| ダッシュボード p95 1.0 秒（NFR 10.1） | リポジトリごとに、最新の判定済み Run と最後の完全計測を `runs` の部分インデックス（`ix_runs_latest`）で 1 行ずつ引く。集計用のテーブルは持たない |
...
ダッシュボード用の集計テーブルは持たない。対象は 1〜5 リポジトリで、リポジトリごとにインデックスで
数行を引くだけで足りる。集計テーブルを持つと、判定のたびの更新と、過去の Run の再評価や
保持期間の削除のあとも最新の状態と整合させる処理が要る。
